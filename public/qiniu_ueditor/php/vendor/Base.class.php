...
class Base{
	...
	public function __call( $method , $params ){
		...
		//var_dump($this->config['upload_type']);
		/*
		if($this->config['upload_type'] == "local"){
			if($method == "getToken"){
				$method = "upload";
			}
		}
		*/
		...
		//var_dump($this->hasUploadMethod( $method ));die;
...
//var_dump(2);die;
	...
	public function setUploadConfig($method){
		...
		$config = array(
            ...
            "rootPath"   => $root_path,
			"config"   => $config,
			"water_url"   => $this->config['water_url'],
			"use_water"   => $this->config['use_water'],
		);
		// scrawl 上传参数
