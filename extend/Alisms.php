...
class Alisms{
    ...
    private $accessKeyId;//账户
	private $accessKeySecret;//密室
	private $signName = '';//签名
	private $templateCode = '';//模板编号
	private $phoneNumbers = '';//手机
	private $templateParam = array();//内容
	private $outId = '';//短信ID
	
	
	
	 public function appkey($appKey=""){
		if($appKey) $this->accessKeyId = $appKey;
		return $this;
	}
	
	public function secret($secretKey=""){
		if($secretKey) $this->accessKeySecret = $secretKey;
		return $this;
	}
    ...
    public function send($signName, $templateCode, $phoneNumbers, $templateParam = null, $outId = null) {
		//p($signName.'---'. $templateCode.'---'. $phoneNumbers);
		//p($templateParam );
		//p($outId);die;
		...
		//p($acsResponse);die;
		$result = (array)$acsResponse;
	//p($result);die;
        if(!empty($result['BizId'])){
		
			Db::name('dayu_sms')->where(array('sms_id' => $outId))->update(array('status' => 1,'info'=>$result['RequestId'].'---'.$result['BizId'].'+++'.$result['Code'].'==='.$result['Message']));
			return true;
		}else{
			Db::name('dayu_sms')->where(array('sms_id' => $outId))->update(array('status' => 0,'info'=>'发送失败【'.$result['Message'].'】'));
			return true;
		}
        return true;
