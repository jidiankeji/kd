...
class Dayu {
	...
	public function send($phone=''){
		...
		if($json['code']){
			...
			Db::name('dayu_sms')->where(array('sms_id' => $sms_id))->update(array('status' => 0,'info'=>'【阿里云】'.$json['info']));
			return $json;
		}else{
			$json['status'] = 1;
			$json['info'] = $json['msg'] ? $json['msg'] : "发送成功";
			Db::name('dayu_sms')->where(array('sms_id' => $sms_id))->update(array('status' => 1,'info'=>'【阿里云】'.$json['info']));
			return $json;
		}
	}
