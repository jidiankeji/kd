...
var uploadErrorInfo = [],
	...
	//登录
	,login: function(){
		location.href = masterDomain + '/wap/passport/login?laiyuan=6';
		return false;
		...
		$("<div>")
			...
			.html('<div class="loadPage">页面加载中，请稍候...</div><iframe></iframe>')//页面加载中，请稍候...
			.appendTo("body");
	}
	...
	,checkLogin: function(fun){
		//异步获取用户信息
		$.ajax({
			url: masterDomain+'/app/api/getUserInfo',
			type: "GET",
			async: false,
			dataType: "jsonp",
			...
			success: function (data) {
				if(data){
					...
					if(data.photo != ""){
						...
						$("#upic").html('欢迎您').css({"width": "auto"});//欢迎您，
					}
					$("#uname").html(data.nickname);

					if(data.message > 0){
						$("#umsg").html("消息("+(data.message > 99 ? "99+" : data.message)+")").show();//消息
					}
					$("#navLoginAfter").show();
				}
			},
	...
	,operaJson: function(url, action, callback){
		$.ajax({
			...
			error: function(){
				...
				$.post("/app/api/checkLogin", "action=checkLogin", function(data){
					if(data == "0"){
						huoniao.showTip("error",'登录超时，请重新登录');//登录超时，请重新登录！
						setTimeout(function(){
							location.reload();
						}, 500);
					}else{
						huoniao.showTip("error",'网络错误，请重试');//网络错误，请重试！
					}
				});
...
$(function(){
	...
	$("body").delegate(".loginconnect, .login-other li a", "click", function(e){
			...
			mtimer = setInterval(function(){
				...
				if($.cookie(cookiePre+"connect_uid") && $.cookie(cookiePre+"connect_code") == type){
					console.log('a')
					loginWindow.close();
					var modal = '<div id="loginconnectInfo"><div class="mask"></div> <div class="layer"> <p class="layer-tit"><span>登录</span></p> <p class="layer-con">'+langData['siteConfig'][20][510]+'<br />'+langData['siteConfig'][38][1].replace('3','<em class="layer_time">3</em>')+'</p> <p class="layer-btn"><a href="'+masterDomain+'/bindMobile.html?type='+type+'">温馨提示 为了您的账户安全，请绑定您的手机号 3s后自动跳转 前往绑</a></p> </div></div>';//温馨提示 为了您的账户安全，请绑定您的手机号 3s后自动跳转 前往绑定
					...
					var timer = setInterval(function(){
						if(t == 1){
							clearTimeout(timer);
							location.href = masterDomain+'/app/api/bindMobile?type='+type;
						}else{
							$(".layer_time").text(--t);
						}
					},1000)
	...
	//引入消息通知公共脚本
	document.getElementsByTagName('head')[0].appendChild(document.createElement('script')).src = masterDomain + '/static/default/wap/new1/js/memberPublicNotice.js?v=' + ~(-new Date());
  ...
  //鼠标经过头部链接显示浮动菜单
	...
	var cityListData = [];
	function getfzCity(){
		$('.changeCityBtn').find('.content ul').html('加载中');//加载中
		$.ajax({
	        url: '/app/api/siteCity?module='+ cfg_module_,
	        type: "GET",
	        ...
	        success: function (data) {
	         if(data && data.state == 100){
	         	...
	         	for(var i = 0; i<datalist.length; i++){
		         	...
		         	html.push('<li><a href="'+datalist[i].url+'" title="'+datalist[i].name+'" class="" data-domain='+JSON.stringify(datalist[i])+'>'+datalist[i].name+'<s><img src="/static/default/wap/new1/images/changecity_curr.png" /></s></a></li>')
	         	}
