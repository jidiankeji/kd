...
class Connect extends Base{
    ...
    public function getConnectByOpenid($type,$open_id){
	   return Db::name('connect')->where(array('type'=>$type,'open_id'=>$open_id))->order(array('connect_id'=>'asc'))->find();     
    }
	
    public function getConnectByUid($uid){
		 return Db::name('connect')->where(array('uid'=>$uid))->find();     
    }
    ...
    public function check_connect_bing($user_id,$type_id){
        ...
        $detail = Db::name('connect')->where(array('uid'=>$user_id,'type'=>$type))->order(array('connect_id'=>'asc'))->find();
        if($detail){
			return true;
        }else{
			return false;	
